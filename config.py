...
- User-editable settings now live in clippy.yaml and environment variables.
- This module loads clippy.yaml at import and exposes familiar globals to avoid breaking imports.
"""

## User-editable defaults are read from clippy.yaml via clippy.config_loader.
## This file holds binary resolution, derived ffmpeg templates, and legacy variable names.

# Load merged config first so globals exist for templates
...
try:
	...
	_merged = {
		# minimal fallbacks; full defaults live in loader
...
# Export merged values as module-level globals to preserve existing imports
...
try:
	...
	if not os.path.isabs(_ff):
		...
		_candidate = os.path.join(os.path.dirname(os.path.abspath(__file__)), _ff)
		if os.path.exists(_candidate):
			fontfile = _candidate
		else:
			_fallback = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'assets', 'fonts', 'Roboto-Medium.ttf')
...
# (YAML overrides already applied at module import)
