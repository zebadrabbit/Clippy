...
def parse_args(argv: Optional[list[str]] = None):
    ...
    p.add_argument(
        "--width", type=int, default=W, help="Output width (default from config or 1920)"
    )
    p.add_argument(
        "--height", type=int, default=H, help="Output height (default from config or 1080)"
    )
    p.add_argument(
        "--fps", type=int, default=FPS, help="Output framerate (default from config or 60)"
    )
    p.add_argument(
        "--prefix", default="transition_", help="Output filename prefix (default transition_)"
    )
