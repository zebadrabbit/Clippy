...
def parse_args() -> argparse.Namespace:
    ...
    g_required.add_argument(
        "--broadcaster",
        help="Broadcaster login name (e.g. theflood). If omitted, uses identity.broadcaster from clippy.yaml if set.",
    )
    g_required.add_argument(
        "--client-id", dest="client_id", help="Twitch Client ID (else TWITCH_CLIENT_ID env)"
    )
    g_required.add_argument(
        "--client-secret", dest="client_secret", help="Twitch Client Secret (else env)"
    )
    ...
    g_window.add_argument(
        "--min-views",
        dest="reactionThreshold",
        type=int,
        default=reactionThreshold,
        help="Minimum view count (maps to reactions)",
    )
    g_window.add_argument(
        "--max-clips", type=int, default=100, help="Max clips to fetch before filtering"
    )
    g_window.add_argument(
        "--clips",
        dest="amountOfClips",
        type=int,
        default=amountOfClips,
        help="Clips per compilation",
    )
    g_window.add_argument(
        "--compilations",
        dest="amountOfCompilations",
        type=int,
        default=amountOfCompilations,
    ...
    g_window.add_argument(
        "--seed", type=int, help="Random seed for reproducible intro/outro/transition selection"
    )
    ...
    g_output.add_argument(
        "--resolution", help="Override output resolution, e.g. 1920x1080 or 1280x720"
    )
    g_output.add_argument(
        "--format", choices=["mp4", "mkv"], help="Container format for final output"
    )
    g_output.add_argument("--fps", type=str, help="Override output framerate, e.g. 60")
    g_output.add_argument(
        "--audio-bitrate", dest="audio_bitrate", type=str, help="Override audio bitrate, e.g. 192k"
    )
    g_output.add_argument(
        "--yt-format", dest="yt_format", type=str, help="yt-dlp --format string override"
    )
    g_output.add_argument(
        "--overwrite-output",
        action="store_true",
        help="Overwrite existing files in output (else auto-suffix _1, _2, ...)",
    )
    ...
    g_trans.add_argument(
        "--intro",
        type=str,
        help="Override: single intro filename (transitions/) to force a specific intro for this run",
    )
    g_trans.add_argument(
        "--outro",
        type=str,
        help="Override: single outro filename (transitions/) to force a specific outro for this run",
    )
    g_trans.add_argument(
        "--transition",
        type=str,
        help="Override: single transition filename used when chosen; static is always placed between segments",
    )
    g_trans.add_argument(
        "--transition-prob",
        dest="transition_prob",
        type=float,
        help="Probability 0..1 to insert a random transition between clips (default from config)",
    )
    g_trans.add_argument(
        "--no-random-transitions",
        action="store_true",
        help="Disable random transitions (keeps static-only between clips)",
    )
    g_trans.add_argument(
        "--transitions-dir",
        dest="transitions_dir",
        type=str,
        help="Path to transitions directory (overrides env and defaults)",
    )
    g_trans.add_argument(
        "--rebuild-transitions",
        action="store_true",
        help="Force re-encode of transitions assets into cache/_trans",
    )
    g_trans.add_argument(
        "--no-audio-normalize-transitions",
        action="store_true",
    ...
    g_perf.add_argument(
        "--max-concurrency", type=int, help="Max workers for download/normalize stage"
    )
    g_perf.add_argument(
        "--skip-bad-clip", action="store_true", help="Skip failed clips instead of aborting"
    )
    g_perf.add_argument("--no-overlay", action="store_true", help="Disable overlay stage for speed")
    g_perf.add_argument(
        "--rebuild", action="store_true", help="Force rebuild even if intermediate files exist"
    )
    ...
    g_discord.add_argument(
        "--discord-channel-id",
        type=int,
        help="Discord channel ID to read from (else clippy.yaml discord.channel_id)",
    )
    g_discord.add_argument(
        "--discord-token", type=str, help="Discord bot token (else DISCORD_TOKEN env or .env)"
    )
    g_discord.add_argument(
        "--discord-limit",
        type=int,
        default=200,
        help="Number of recent messages to scan for clip links",
    )
