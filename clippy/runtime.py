...
def ensure_twitch_credentials_if_needed():
    ...
    if not cid or not secret:
        ...
        log(
            "  - PowerShell env vars: $env:TWITCH_CLIENT_ID='...'; $env:TWITCH_CLIENT_SECRET='...'",
            1,
        )
...
def ensure_transitions_static_present(transitions_dir: Optional[str] = None):
    ...
    if not os.path.isdir(tdir):
        ...
        _log(
            "Place your clips (intro.mp4, static.mp4, outro.mp4) in this folder or provide --transitions-dir.",
            1,
        )
        raise SystemExit(2)
    if not os.path.exists(static_path):
        ...
        _log(
            "This project requires transitions/static.mp4. Set TRANSITIONS_DIR, use --transitions-dir, or place the file.",
            1,
        )
        raise SystemExit(2)
